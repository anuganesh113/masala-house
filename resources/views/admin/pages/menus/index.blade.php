            ...
            <table class="m-datatable" id="html_table" width="100%">
                ...
                <tbody>
                    ...
                    @foreach($menus ?? [] as $value)
                        <tr id="menus-{{ $value->id }}">
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ ucfirst(data_get($value, 'category.name')) }}</td>
                            <td>{{ data_get($value, 'name') }}</td>
                            <td>{{ !empty(data_get($value, 'image')) ? 'Image Found' : 'Image Not Found' }}</td>
                            <td>
                                <span class="text-{{ data_get($value, "status") ? 'success' : 'danger' }}">
                                    {{ data_get($value, 'status') ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td>
                                <span class="text-{{ data_get($value, 'type') == \App\Enums\FoodType::VEG ? 'success' : 'danger' }}">
                                    {{ data_get($value, 'type') }}
                                </span>
                            </td>
