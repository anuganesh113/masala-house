        ...
        <div class="m-portlet__body">
            <div class="m-form m-form--label-align-right m--margin-top-20 m--margin-bottom-30">
                <div class="row align-items-center">
                    ...
                    <div class="col-xl-4 order-1 order-xl-2 m--align-right">
                        <a href="{{ route('admin.pages.create') }}" class="btn btn-primary m-btn m-btn--custom m-btn--icon m-btn--air m-btn--pill">
                            <span>
            ...
            <table class="m-datatable" id="html_table" width="100%">
                ...
                @foreach($pages??[] as $value)
                    <tr id="pages-{{ data_get($value, 'id') }}">
                        <td>{{ $loop->iteration }}</td>
                        <td>{{ data_get($value, 'name') }}</td>
                        <td>{{ !empty(data_get($value, "image_one")) ? "Image Found" : "Image Not Found" }}</td>
                        <td>{{ $value->order }}</td>
                        <td>
                            @if(!isset($value->parent))
                                <span class="m-badge m-badge--success m-badge--wide">No Parent Page</span>
                            @else
                                <span class="m-badge m-badge--warning m-badge--wide">
                                    {{ ucwords(data_get($value, 'parent.name')) }}
                                </span>
                            @endif
                        </td>
                        <td>
                        ...
                        <td data-field="Actions" class="m-datatable__cell">
                            <span style="overflow: visible; width: 110px;">
                                <a href="{{ route('admin.pages.edit', data_get($value, 'id')) }}"
                                ...
                                <a href="javascript:;" onclick="deletePages({{ data_get($value, 'id') }})"
