@section("page_title", "Advertise List")
            ...
            <div class="m-portlet__body">
                <div class="m-form m-form--label-align-right m--margin-top-20 m--margin-bottom-30">
                    <div class="row align-items-center">
                        ...
                        <div class="col-xl-4 order-1 order-xl-2 m--align-right">
                            <a href="{{ route('admin.advertises.create') }}"
                ...
                <table class="m-datatable" id="html_table" width="100%">
                    ...
                    @foreach($advertises ?? [] as $value)
                        <tr id='advertises-{{ $value->id }}'>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ data_get($value, 'name') }}</td>
                            <td>{{ data_get($value, 'pics') !== null ? 'Image Found' : 'Image Not Found' }}</td>
                            <td>{{ data_get($value, 'order') }}</td>
                            <td>{{ ucwords(data_get($value, 'type')) }}</td>
                            <td>
                                <span class="m-badge m-badge--{{ data_get($value, "status", 0) ? 'success' : 'danger' }} m-badge--wide">
                            ...
                            <span style="overflow: visible; width: 110px;">
                                <a href="{{ route('admin.advertises.edit', $value->id) }}"
                                ...
                                <a href="javascript:;" onclick="deleteAdvertises({{ $value->id }})"
